...
! VARIABLES DECLARATION
  ...
  real(8), parameter :: aa = 0.7, bb = 1.5, cc = 1.108
...
!----------------------------------------------------------------------!
  ...
  ! now ck and cex is no longer previous constant, but fixed by us

  ! initialization
  cex = 1.337d0
  ck = 0.108
  open(3,file="kathatacheck.dat")
  do l = 1, nlinei
     y(l) = ck*(x(l) - cex)
     optkapp(l) = y(l) + 1.d0
     ...
     write(3,"(3f7.3)"), dt(l), Ecal(l), Eexp(l)
  end do
  close(3)
